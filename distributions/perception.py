...
def uniform(N, K, weights, competences):
  return np.random.default_rng().uniform(low=0.0, high=1.0, size=(N,N))

def normal(N, K, weights, competences):
...
def true(N, K, weights, competences):
...
def knowledgePerception(N, K, weights, competences):
  NORNALIZATION = 0.7 
  THRESHOLD = 0.2
...
def competence_favor_neighbors(N, K, weights, competences):
    perceptions = np.zeros((N,N))   
    for i in range(N):
        for j in range(N):
            if i == j:
